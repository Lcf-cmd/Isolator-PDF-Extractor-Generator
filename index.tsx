...
const App = () => {
  ...
  const handlePdfFolderUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files) return;
    const pdfs = Array.from(files).filter((f: File) => f.name.toLowerCase().endsWith('.pdf'));
    setRawPdfs(pdfs);
  };
  ...
  const handleImageFolderUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files) return;
    const imgs = Array.from(files).filter((f: File) => /\.(png|jpe?g)$/i.test(f.name));
    setRawImages(imgs);
  };
  ...
  const extractPagesAsImages = async (pdfFile: File): Promise<string[]> => {
    ...
    for (let i = 1; i <= Math.min(pdf.numPages, 8); i++) {
      ...
      const viewport = page.getViewport({ scale: 3.0 }); // Increased scale for better chart resolution
  ...
  const sanitizeText = (text: string) => {
    if (!text) return "";
    return text
      .replace(/℃/g, '°C').replace(/uA/g, 'µA').replace(/OHM/gi, 'Ω')
      .replace(/±/g, '±').replace(/≤/g, '≤').replace(/≥/g, '≥')
      .replace(/[фΦ¦Ææ\u00C6\u00E6\u00D8\u00F8\u0152\u0153]/g, '') // Remove special symbols that cause corruption in standard PDF fonts
      .replace(/[^\x00-\x7F°µΩ±≤≥]/g, '') // More aggressive removal of non-ASCII characters that aren't explicitly allowed
  ...
  const formatProductId = (product: ProductData) => {
    const { modelNumber, sizeSpec, freqRange } = product;
    let baseId = "";
    
    // Clean inputs
    const cleanModel = (modelNumber || "").trim();
    const cleanSize = (sizeSpec || "").replace(/[фΦ¦Ææ]/g, '').trim();
    const cleanFreq = (freqRange || "").trim();

    if (cleanModel && !cleanModel.toLowerCase().includes("circulator") && !cleanModel.toLowerCase().includes("isolator")) {
      // Rule: O + 型号 + - + 工作频率
      baseId = "O" + cleanModel;
      if (cleanFreq) {
        baseId += "-" + cleanFreq;
      ...
      // Rule: O- + 尺寸规格 (cleaned) + - + 工作频率
      baseId = "O-" + (cleanSize || "Product");
      if (cleanFreq) {
        baseId += "-" + cleanFreq;
      }
    }

    // Ensure all underscores are replaced with hyphens as per user request
    return (baseId || "Unknown-Product").replace(/_/g, '-');
  };
  ...
  const runSynthesis = async (pdfFile: File, assets: ManualAsset[]) => {
    ...
    const parts = [
      { text: `TASK: Extract Product Specs. 
      IMPORTANT: If the PDF page contains a summary table (Technical Indicators), EACH ROW in that table represents a UNIQUE product variant. 
      - You MUST generate a separate entry in the 'products' array for EVERY ROW in the table.
      - Each entry must have its own 'modelNumber' (from the '型号' or 'Part Number' column) and 'freqRange' (from the '工作频率' or 'Freq. (MHz)' column).
      - If multiple rows share the same 'modelNumber' but have different 'freqRange', they are STILL separate products and must be split.
      - The 'specsTable' for each product entry should contain ONLY that single row from the original table (plus the headers).
      
      Extraction Rules for each product entry:
      - modelNumber: The value from '型号' or 'Part Number' column. If this column does not exist or is empty for a row, leave this field EMPTY or NULL. DO NOT put the product title here.
      - sizeSpec: The value from '尺寸规格' or 'Size Spec' column (e.g., 'ф5mm').
      - freqRange: The value from '工作频率' or 'Freq. (MHz)' column (e.g., '2496-2690').
      - productName: The main title of the product section (e.g., 'Stripline Surface Mount Circulator').
      - description: A concise paragraph describing the product category. IMPORTANT: You MUST start the description with the phrasing: "The product named “{ID}” is high-performance..." where {ID} is the specific product ID (e.g., 'O-5mm-4800-5000' or 'OH121A-1805-1880').
      - features: A list of key features.
      - Tables: Extract specsTable (single row), environmentalParams, absoluteMaxRatings.
      
      CRITICAL: For 'imageBoxes.curves', you MUST identify EACH INDIVIDUAL chart/graph separately. Do NOT group multiple charts into one box. If there are 4 charts in a row, return 4 separate boxes in the 'curves' array.
      
      CRITICAL: For 'specsTable' and other tables, DO NOT use garbled characters (like 'Æ', 'Φ', 'ф', etc.). Map them to standard English terms or remove them if they are noise.
      
      CRITICAL: For 'specsTable' headers, DO NOT use garbled characters. Map them to these standard terms:
      "Size Spec", "Freq. (MHz)", "Return Loss (dB)", "Isolation (dB)", "Insertion Loss (dB)", "IMD3 (dBc)", "Power (W)", "Operating Temp (°C)".
      
      Global fields (shared across all products on the page):
            ...
            products: {
              ...
              items: {
                ...
                properties: {
                  ...
                  sizeSpec: { type: Type.STRING },
                  freqRange: { type: Type.STRING },
    ...
    for (const product of data.products) {
      const finalId = formatProductId(product);
      const formattedProduct = {
        ...product,
        modelNumber: finalId // Using the formatted ID as the primary identifier in the PDF
      };
      generateFinalPDF(formattedProduct, finalMains, finalOutlines, finalCurves);
    }
  };
  ...
  const generateFinalPDF = (data: ProductData, mainImgs: string[], outlineImgs: string[], curves: string[]) => {
    // @ts-ignore
    ...
    const addHeaderFooter = () => {
      ...
      doc.setFontSize(10);
      doc.setTextColor(60, 60, 60); // Professional dark gray
      doc.setFont("helvetica", "bold");
      doc.text(`ID: ${sanitizeText(data.modelNumber || "")}`, pageWidth - margin, 18, { align: 'right' });
    ...
    // Use the formatted ID as the main title
    const titleText = data.modelNumber || "Product";
    ...
    if (outlineImgs.length > 0) {
      if (currentY > pageHeight - 120) { doc.addPage(); addHeaderFooter(); currentY = 35; }
      doc.setFontSize(14); doc.setFont("helvetica", "bold");
      doc.text("Mechanical Outline", margin, currentY);
      currentY += 10;
      outlineImgs.forEach((img) => {
        // Increased max size for Outline images
        const { w, h } = getImageDimensions(img, pageWidth - (margin * 2), 100);
    ...
    if (curves.length > 0) {
      ...
      const rowHeight = 75; // Increased row height for larger images
      let startY = 45;
      
      curves.forEach((c, idx) => {
        const col = idx % 2;
        const pageIdx = Math.floor(idx / 4);
        const inPageIdx = idx % 4;
        const row = Math.floor(inPageIdx / 2);
        ...
        if (idx > 0 && idx % 4 === 0) {
          ...
          addHeaderFooter();
          doc.setFontSize(14); doc.setFont("helvetica", "bold");
          doc.text("Performance Data (cont.)", margin, 35);
        }

        const x = margin + col * (colWidth + cGap);
        const y = startY + row * (rowHeight + cGap);
        
        const { w, h } = getImageDimensions(c, colWidth, rowHeight);
        const centeredX = x + (colWidth - w) / 2;
        const centeredY = y + (rowHeight - h) / 2;
        
        doc.addImage(c, 'PNG', centeredX, centeredY, w, h);
      });
    }

    doc.save(`${sanitizeText(data.modelNumber || 'Datasheet')}.pdf`);
  };
      ...
      <header className="mb-14 flex justify-between items-center border-b pb-8 border-slate-100">
        <div className="flex-1">
          <h1 className="text-5xl font-black text-slate-900 tracking-tighter italic uppercase flex items-center">
            Huayang Builder 
            <span className="bg-slate-900 text-white text-[10px] font-bold not-italic px-3 py-1 rounded-full ml-4 tracking-normal uppercase">Robust v7.0</span>
          </h1>
          <p className="text-slate-400 font-semibold mt-2 tracking-wide uppercase">Huayang Communication Synthesis Engine</p>
        </div>
